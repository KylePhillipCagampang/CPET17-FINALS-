...
app.post("/api/camera/insert", (req, res) => {
  // create query to db
  connection.execute(
    "INSERT INTO `cameradetect` (start_time, end_time, image) VALUES (?, ?, ?)",
...
app.get("/api/motion/:id", (req, res) => {
  ...
  // create query to db
  connection.execute(
    "SELECT * FROM `cameradetect` WHERE id = ?",
    [id],
...
app.post("/api/users/reset_pass", (req, res) => {
  ...
  var transporter = nodemailer.createTransport({
    service: "hotmail",
    auth: {
      user: "cpet17finals@outlook.com",
      pass: "Nettoysfinals",
    },
  });

  var mailOptions = {
    from: "cpet17finals@outlook.com",
    to: email,
    subject: "CPET17FINALS - Reset pasword link",
    text: link,
  };
